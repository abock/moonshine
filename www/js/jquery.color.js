 ...
 * 
 * Modifications for rgbA by Philipp Schumann.
 * begin with / *BEGIN MOD* /
 * end with / *END MOD* /
 */
...
((jQuery => {
	...
	jQuery.each(['backgroundColor', 'borderBottomColor', 'borderLeftColor', 'borderRightColor', 'borderTopColor', 'color', 'outlineColor'], (i, attr) => {
		jQuery.fx.step[attr] = fx => {
			...
			if ((fx.start.length > 3) || (fx.end.length > 3))
				fx.elem.style[attr] = "rgba(" + [
				                				Math.max(Math.min( parseInt((fx.pos * (fx.end[0] - fx.start[0])) + fx.start[0]), 255), 0),
				                				Math.max(Math.min( parseInt((fx.pos * (fx.end[1] - fx.start[1])) + fx.start[1]), 255), 0),
				                				Math.max(Math.min( parseInt((fx.pos * (fx.end[2] - fx.start[2])) + fx.start[2]), 255), 0),
				                				Math.max(Math.min( parseFloat((parseFloat(fx.pos) * ((fx.end[3] || 0.0) - (fx.start[3] || 0.0))) + (fx.start[3] || 0.0)), 1.0), 0)
				                			].join(",") + ")";
			else
				/*END MOD*/
	...
	function getRGB(color) {
		...
		// Look for rgba(num,num,num,0.num)
		if (result = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\,\s*(0.[0-9]{1,3})\s*\)/.exec(color))
			return [parseInt(result[1]), parseInt(result[2]), parseInt(result[3]), parseFloat(result[4])];

		// Look for rgba(num%,num%,num%,0.num)
		if (result = /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\,\s*(0.[0-9]{1,3})\s*\)/.exec(color))
